...
function boot() {
  ...
  async function fetchData() { try { const r = await fetch(DEX_API); const d = await r.json(); const p = d?.pairs?.[0]; if (!p) return; livePrice = +p.priceUsd || 0; const mc = +p.marketCap || 0, ch = +(p.priceChange?.h24) || 0; try { (window as any).zappChange = ch; } catch {} const fmt = (v: number) => v >= 1e9 ? '$' + (v / 1e9).toFixed(2) + 'B' : v >= 1e6 ? '$' + (v / 1e6).toFixed(1) + 'M' : v >= 1e3 ? '$' + (v / 1e3).toFixed(0) + 'K' : '$' + v.toFixed(0); const fp = (v: number) => v < 0.0001 ? v.toFixed(9) : v < 0.01 ? v.toFixed(6) : v.toFixed(4); const pStr = '$' + fp(livePrice); const set = (id: string, t: string) => { const e = document.getElementById(id); if (e) e.textContent = t; }; set('p-price', pStr); set('p-mcap', fmt(mc)); const ch2 = document.getElementById('p-change'); if (ch2) { ch2.textContent = (ch >= 0 ? '+' : '') + ch.toFixed(2) + '%'; (ch2 as HTMLElement).style.color = ch >= 0 ? '#10B981' : '#EF4444'; } set('zv', fmt(mc)); set('zv2', fmt(mc)); set('dn-price', '⚡ ' + pStr); uc(); } catch { /* offline */ } }
  const ci = cinEl();
