...
function startFuture() {
  ...
  earth.add(new THREE.Mesh(new THREE.SphereGeometry(R, 40, 28), new THREE.MeshBasicMaterial({ color: 0x12A04E, wireframe: true, transparent: true, opacity: 0.22 })));
  const atmo = new THREE.Mesh(new THREE.SphereGeometry(R * 1.18, 36, 26), new THREE.MeshBasicMaterial({ color: 0x2BFF77, transparent: true, opacity: 0.1, side: THREE.BackSide, blending: THREE.AdditiveBlending })); earth.add(atmo);
  ...
  const pal = [new THREE.Color(0x7BFFA8), new THREE.Color(0x2BFF77), new THREE.Color(0x15C957), new THREE.Color(0xC8FFDD), new THREE.Color(0x12A04E)]; // emerald greens — the living network
  ...
  for (let i = 0; i < 16; i++) {
    ...
    const m = new THREE.Mesh(new THREE.CylinderGeometry(0.02, 0.09, 0.95, 6), new THREE.MeshBasicMaterial({ color: 0xC8FFDD }));
    ...
    const glow = new THREE.Mesh(new THREE.SphereGeometry(0.13, 8, 8), new THREE.MeshBasicMaterial({ color: 0xC8FFDD, transparent: true, opacity: 0.9, blending: THREE.AdditiveBlending })); glow.position.copy(tip); earth.add(glow);
  }
  ...
  function makeArc(a: THREE.Vector3, b: THREE.Vector3) {
    ...
    earth.add(new THREE.Line(new THREE.BufferGeometry().setFromPoints(cpts), new THREE.LineBasicMaterial({ color: 0x2BFF77, transparent: true, opacity: 0.3, blending: THREE.AdditiveBlending })));
